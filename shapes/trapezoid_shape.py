...
class TrapezoidShape(BaseShape):
    ...
    @staticmethod
    def build_isosceles(base_a: float, base_b: float, angle_deg: float) -> List[Tuple[float, float]]:
    ...
    def get_handle_type(self, point: QPointF, tolerance: float = 5.0) -> HandleType:
        for i, v in enumerate(self._vertices):
            ...
            if distance <= tolerance:
                handle_map = [HandleType.TOP_LEFT, HandleType.BOTTOM_RIGHT, 
                             HandleType.BOTTOM_LEFT, HandleType.TOP_RIGHT]
                return handle_map[i]
